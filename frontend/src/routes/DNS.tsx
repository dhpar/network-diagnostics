...
function DNS() {
    ...
    return (
        <Layout title='DNS lookup' isRefreshLoading={scanDNS.isLoading || scanDNS.isRefetching} refetch={scanDNS.refetch}>
            <Card className="mb-4 inline-flex">
                <TriangleAlert className="fill-transparent mr-4" stroke={`var(--color-red-400)`} />
                <p> Note: time to ping the host DNS and return back (RTT)</p>
            </Card>
            <div className="space-y-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            { scanDNS.data?.map((result, idx) => (
                <Card key={idx} className="grid grid-cols-2 grid-rows-2 gap-2 h-full">
